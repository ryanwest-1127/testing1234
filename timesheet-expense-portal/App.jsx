...
function Dashboard({ visibleClaims, categoryData, totals, weeklyStandardHours, setWeeklyStandardHours, activeUser, selectedWeek }) {
  ...
  const selectedTimesheetClaims = dashboardWeek === 'current'
    ? cumulativeTimesheetClaims
    : timesheetClaims.filter(c => c.week === dashboardWeek);

  const selectedExpenseClaims = dashboardWeek === 'current'
    ? cumulativeExpenseClaims
    : expenseClaims.filter(c => c.week === dashboardWeek);

  const totalSubmittedHours = selectedTimesheetClaims.reduce((s, c) => s + Number(c.totals?.totalWorkingHours || 0), 0);
  ...
  const totalExpenses = selectedExpenseClaims.reduce((s, c) => s + Number(c.totals?.totalExpense || 0), 0);
  ...
  const targetHours = dashboardWeek === 'current'
    ? Number(weeklyStandardHours || 0) * new Set(selectedTimesheetClaims.map(c => c.week)).size
    : Number(weeklyStandardHours || 0);
    ...
    <div className="space-y">
      <div className="card">
        <div className="card-content flex gap items-center" style={{ flexWrap: 'wrap' }}>
          ...
          <select className="select" value={dashboardWeek} onChange={e => setDashboardWeek(e.target.value)} style={{ width: 300 }}>
            <option value="current">Current data: Week 1 to {weekLabel(currentWeekLimit)}</option>
            {weekOptions.map(week => (
              <option key={week} value={week}>{weekLabel(week)}</option>
            ))}
          </select>
          <p className="small muted">
            {dashboardWeek === 'current'
              ? 'Cumulative total from Week 1 up to the selected/current week.'
              : 'Selected week only.'}
          </p>
        </div>
      </div>
            ...
            <div>
              ...
              <p className="xsmall muted">
                {dashboardWeek === 'current'
                  ...
                  : 'Selected week actual / weekly target'}
              </p>
              {activeUser?.role === 'Manager' && (
      ...
      <div className="grid grid-3">
        <Insight
          ...
          note={dashboardWeek === 'current' ? 'Cumulative expense claims' : 'Selected week expense claims'}
          icon={<WalletCards />}
        />
        <Insight
