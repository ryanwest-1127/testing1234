...
function Dashboard({ visibleClaims, categoryData, totals, weeklyStandardHours, setWeeklyStandardHours, activeUser, selectedWeek }) {
  ...
  const weekOnlyTimesheetClaims = timesheetClaims.filter(c => c.week === selectedWeekOnly);
  const weekOnlyExpenseClaims = expenseClaims.filter(c => c.week === selectedWeekOnly);

  const timeInLieuThisWeek = weekOnlyTimesheetClaims.reduce(
    (s, c) => s + Number(c.totals?.timeInLieu || 0) - Number(c.totals?.takeBackTimeInLieu || 0),
    0
  );
  const expenseThisWeekTotal = weekOnlyExpenseClaims.reduce((s, c) => s + Number(c.totals?.totalExpense || 0), 0);
  const expenseThisWeekApprovedPaid = weekOnlyExpenseClaims
    .filter(c => ['Approved', 'Paid'].includes(c.status))
    .reduce((s, c) => s + Number(c.totals?.totalExpense || 0), 0);
  const expenseThisWeek = Math.max(0, expenseThisWeekTotal - expenseThisWeekApprovedPaid);
  ...
  // Time in Lieu Remaining should be recalculated from all records up to the selected week.
  // Formula: cumulative Time in Lieu earned - cumulative Take back Time in Lieu.
  const latestTilBalance = selectedTimesheetClaims.reduce(
    (s, c) => s + Number(c.totals?.timeInLieu || 0) - Number(c.totals?.takeBackTimeInLieu || 0),
    0
  );
  ...
  return (
    <div className="space-y">
      <div>
        ...
        <p className="small muted">Use this dashboard for cumulative totals and selected-week data including TIL this week and expense this week.</p>
      </div>
      <div className="card">
        <div className="card-content flex gap items-center" style={{ flexWrap: 'wrap' }}>
          <label className="label">Detailed dashboard data</label>
      ...
      <div className="grid grid-3">
        ...
        <Insight
          ...
          note="Cumulative TIL earned minus take back TIL"
          icon={<FileCheck2 />}
        />
        <Insight
      ...
      <div className="grid grid-3">
        ...
        <Insight
          ...
          value={money(expenseThisWeek)}
          note={`${weekLabel(selectedWeekOnly)} outstanding`}
          icon={<WalletCards />}
        />
        <Insight
          title="Expense This Week Approved / Paid"
          value={money(expenseThisWeekApprovedPaid)}
          note={`${weekLabel(selectedWeekOnly)} manager confirmed`}
          icon={<CheckCircle2 />}
        />
      </div>
