...
function calculateTakeBackTimeInLieuHours(row, standardHours) {
  const dailyHours = Number(standardHours || 7.5);

  if (row.takeBackTimeInLieu === true) return dailyHours;
...
function isLeaveDay(row) {
  return row.holidayType === 'half' || row.holidayType === 'full' || row.sickness === true || row.takeBackTimeInLieu === true;
}
...
function calculateTotals(timesheet, expenses, til, standardHours) {
  ...
  weekDays.forEach(day => {
    ...
    t.takeBackTimeInLieu += calculateTakeBackTimeInLieuHours(row, standardHours);
            ...
            <table>
              ...
              <tbody>
                {weekDays.map(day => {
                  ...
                  const takeBackSelected = row.takeBackTimeInLieu === true;
                  const leaveSelected = holidayType === 'half' || holidayType === 'full' || sicknessSelected || takeBackSelected;
                  ...
                  const takeBackHours = takeBackSelected ? dailyHours : 0;
                  const sicknessHours = sicknessSelected ? dailyHours : 0;
                  const autoTimeInLieu = leaveSelected ? 0 : Math.max(0, workedHours - dailyHours);
                  const total = leaveSelected ? holidayHours + takeBackHours + sicknessHours : workedHours;
                  ...
                  const setHoliday = (type) => {
                    ...
                    p.updateTimesheet(day, 'takeBackTimeInLieu', false);
                  ...
                  const setTakeBackTimeInLieu = () => {
                    const nextValue = !takeBackSelected;
                    p.updateTimesheet(day, 'takeBackTimeInLieu', nextValue);
                    p.updateTimesheet(day, 'holidayType', 'none');
                    p.updateTimesheet(day, 'sickness', false);
                    if (nextValue) clearWorkingInputs();
                  };
                    ...
                    <tr key={day}>
                      ...
                      <td>
                        <label className="small" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <input
                            type="checkbox"
                            checked={takeBackSelected}
                            onChange={setTakeBackTimeInLieu}
                          />
                          Take back ({dailyHours.toFixed(2)}h)
                        </label>
                      </td>
