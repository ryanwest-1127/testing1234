...
function Dashboard({ visibleClaims, categoryData, totals, weeklyStandardHours, setWeeklyStandardHours, activeUser }) {
  ...
  const totalSubmittedHours = timesheetClaims.reduce((s, c) => s + Number(c.totals?.totalWorkingHours || 0), 0);
  ...
  timesheetClaims.forEach(c => {
    timesheetCategoryMap['Project Hours'] += Number(c.totals?.projectHours || 0);
    timesheetCategoryMap.Travel += Number(c.totals?.travelHours || 0);
    timesheetCategoryMap.Workshop += Number(c.totals?.workshopHours || 0);
  ...
  const selectedCategoryData = categoryView === 'timesheet' ? timesheetCategoryData : categoryData;
  const chartValueFormatter = categoryView === 'timesheet'
    ? (value) => `${Number(value || 0).toFixed(2)} hrs`
    : (value) => money(value);

  const currentHours = totals.totalWorkingHours;
        ...
        <div className="card">
          <div className="card-content flex gap items-center">
            ...
            <div>
              ...
              <p className="metric-value">{currentHours.toFixed(2)} / {targetHours.toFixed(2)} hrs</p>
              <p className="xsmall muted">Current week / weekly target</p>
              {activeUser?.role === 'Manager' && (
