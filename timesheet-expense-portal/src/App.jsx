...
function Dashboard({ visibleClaims, categoryData, totals, weeklyStandardHours, setWeeklyStandardHours, activeUser, selectedWeek }) {
  ...
  const selectedTimesheetClaims = dashboardWeek === 'current'
    ? []
    : timesheetClaims.filter(c => c.week === dashboardWeek);
  const selectedExpenseClaims = dashboardWeek === 'current'
    ? []
    : expenseClaims.filter(c => c.week === dashboardWeek);

  const displayedTimesheetClaims = dashboardWeek === 'current' ? [] : selectedTimesheetClaims;
  const displayedExpenseClaims = dashboardWeek === 'current' ? [] : selectedExpenseClaims;
  const totalSubmittedHours = dashboardWeek === 'current'
    ? totals.totalWorkingHours
    : displayedTimesheetClaims.reduce((s, c) => s + Number(c.totals?.totalWorkingHours || 0), 0);
  const totalAutoTil = dashboardWeek === 'current'
    ? totals.timeInLieu
    : displayedTimesheetClaims.reduce((s, c) => s + Number(c.totals?.timeInLieu || 0), 0);
  const latestTilBalance = dashboardWeek === 'current'
    ? totals.tilBalance
    : (displayedTimesheetClaims[0]?.totals?.tilBalance ?? 0);
  const totalExpenses = dashboardWeek === 'current'
    ? 0
    : displayedExpenseClaims.reduce((s, c) => s + Number(c.totals?.totalExpense || 0), 0);
  const approvedPaidExpenses = dashboardWeek === 'current'
    ? 0
    : displayedExpenseClaims
  ...
  const sourceTimesheetClaims = dashboardWeek === 'current'
    ? [{ totals }]
    : displayedTimesheetClaims;

  sourceTimesheetClaims.forEach(c => {
  ...
  displayedExpenseClaims.forEach(c => c.expenses?.forEach(e => {
    selectedExpenseCategoryMap[e.category] = (selectedExpenseCategoryMap[e.category] || 0) + Number(e.amount || 0);
  }));
  const selectedExpenseCategoryData = dashboardWeek === 'current'
    ? []
    : Object.entries(selectedExpenseCategoryMap).map(([name, value]) => ({ name, value }));
    ...
    <div className="space-y">
      <div className="card">
        <div className="card-content flex gap items-center" style={{ flexWrap: 'wrap' }}>
          ...
          <select className="select" value={dashboardWeek} onChange={e => setDashboardWeek(e.target.value)} style={{ width: 260 }}>
            <option value="current">Current form data ({weekLabel(selectedWeek)})</option>
      ...
      <div className="grid grid-3">
        <div className="card">
          <div className="card-content flex gap items-center">
            ...
            <div>
              ...
              <p className="xsmall muted">{dashboardWeek === 'current' ? 'Current form / weekly target' : 'Selected week / weekly target'}</p>
              {activeUser?.role === 'Manager' && (
        ...
        <Insight
          ...
          note="Submitted timesheet TIL total"
          icon={<TrendingUp />}
        />
      </div>
      ...
      <div className="grid grid-3">
        ...
        <Insight
          title="Records Count"
          value={String(dashboardWeek === 'current' ? 0 : displayedTimesheetClaims.length + displayedExpenseClaims.length)}
          note={dashboardWeek === 'current' ? 'Current form is not submitted yet' : 'Timesheet + expense records'}
          icon={<CalendarDays />}
        />
      </div>
      ...
      <ChartCard title="Categories" sub={categoryView === 'timesheet' ? 'Timesheet hours breakdown' : 'Expense claim breakdown'}>
        <div className="flex gap items-center" style={{ flexWrap: 'wrap', marginBottom: 16 }}>
          <select className="select" value={categoryView} onChange={e => setCategoryView(e.target.value)} style={{ width: 220 }}>
            <option value="timesheet">Timesheet categories</option>
            <option value="expense">Expense categories</option>
          </select>

          <select className="select" value={chartType} onChange={e => setChartType(e.target.value)} style={{ width: 160 }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={selectedCategoryData} dataKey="value" nameKey="name" outerRadius={95} label>
                {selectedCategoryData.map((_, i) => <Cell key={i} />)}
              </Pie>
