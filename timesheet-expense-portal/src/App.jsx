...
function Dashboard({ visibleClaims, categoryData, totals, weeklyStandardHours, setWeeklyStandardHours, activeUser, selectedWeek }) {
  ...
  const weekCount = new Set(selectedTimesheetClaims.map(c => c.week)).size;
  const targetHours = Number(weeklyStandardHours || 0) * (weekCount || 1);
      ...
      <div className="card">
        <div className="card-content flex gap items-center" style={{ flexWrap: 'wrap' }}>
          ...
          <select className="select" value={dashboardWeek} onChange={e => setDashboardWeek(e.target.value)} style={{ width: 330 }}>
            <option value="current">Current data: Week 1 to {weekLabel(selectedWeek || latestSubmittedWeek)}</option>
            {weekOptions.map(week => (
              <option key={week} value={week}>Week 1 to {weekLabel(week)}</option>
            ))}
          </select>
          <p className="small muted">
            All totals are cumulative from Week 1 up to the selected week.
          </p>
        </div>
      </div>
        ...
        <div className="card">
          <div className="card-content flex gap items-center">
            ...
            <div>
              ...
              <p className="xsmall muted">Cumulative actual / cumulative target</p>
              {activeUser?.role === 'Manager' && (
